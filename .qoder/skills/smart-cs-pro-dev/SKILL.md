...
### 全局影响评估与协同进化 (Holistic Impact & Evolution)
在完成任何新功能的开发或 UI 更新后，**必须执行以下自检与优化动作**：
1. **整体功能协同检查**: 评估新功能是否能为其他模块提供价值。
   - *案例*: 新增了坐席雷达图数据，应主动检查主管端大屏是否需要同步显示这些维度。
2. **数据链闭环验证**: 检查新功能产生的日志或指标是否已接入 `AuditManager` 和 `DataSyncer`。
3. **架构冗余清理**: 如果新功能替换了旧逻辑（如用雷达图替换了饼图），必须彻底清理无用代码并重构相关组件。
4. **自驱动优化**: 若发现新功能可以通过微调现有组件（如修改全局 CSS 变量或 Zustand 状态）获得更好的系统级集成效果，应**自动执行**此类优化，无需等待进一步指令。
