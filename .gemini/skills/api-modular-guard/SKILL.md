...
*   **重要操作审计红线 (强制检查)**：**系统必须识别并记录所有“重要操作”至物理审计日志 (`AuditLog`)**。重要操作定义如下：
    *   **所有写操作**：涉及业务实体的 `CREATE` (增), `UPDATE` (改), `DELETE` (删)。
    *   **权限变更**：角色分配、权责矩阵调整、登录状态人工强制注销。
    *   **战术指令**：物理锁定输入、远程指令弹射、语音告警下发。
    *   **物理同步**：大规模数据导入（CSV/Excel）、全局策略重载。
    *   **审计要求**：日志必须包含操作人真实姓名、动作类型、目标对象标识及具体的变更载荷。
