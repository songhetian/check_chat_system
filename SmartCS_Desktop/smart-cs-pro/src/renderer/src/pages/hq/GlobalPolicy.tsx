...
export default function GlobalPolicyPage() {
  ...
  const handleSave = async () => {
    ...
    if (res.data.status === 'ok') { setModalType('NONE'); fetchData(); }
  }

  const executeDelete = async () => {
    if (!editItem || !token) return
    setModalType('NONE'); fetchData();
    window.dispatchEvent(new CustomEvent('trigger-toast', { detail: { title: '策略已移除', message: '战术规则已重载', type: 'success' } }))
  }
                ...
                <TacticalTable headers={['敏感词', '所属分类', '风险权重', '状态', '战术调整']}>
                  {words.map(w => (
                    <tr key={w.id} className="hover:bg-slate-50/50 transition-colors group text-center text-sm font-bold text-slate-600">
                      ...
                      <td className="px-6 py-5"><div className="flex justify-center items-center gap-2"><div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden"><div className={cn("h-full transition-all", w.risk_level >= 8 ? "bg-red-500" : w.risk_level >= 5 ? "bg-amber-500" : "bg-cyan-500")} style={{ width: `${w.risk_level * 10}%` }} /></div><span className="text-xs font-black italic">LV.{w.risk_level}</span></div></td>
                      ...
                      <td className="px-8 py-5 text-center"><div className="flex justify-center gap-2"><button onClick={() => { setEditItem(w); setModalType('EDIT') }} className="p-2.5 bg-slate-50 text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-xl transition-all shadow-sm"><Edit3 size={16} /></button><button onClick={() => { setEditItem(w); setModalType('DELETE') }} className="p-2.5 bg-slate-50 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all shadow-sm"><Trash2 size={16} /></button></div></td>
                    </tr>
                  ))}
                </TacticalTable>
              ) : (
                <TacticalTable headers={['触发关键词', '建议内容', '业务分类', '状态', '战术调整']}>
                  {kb.map(k => (
                    <tr key={k.id} className="hover:bg-slate-50/50 transition-colors group text-sm font-bold text-slate-600 text-center">
                      ...
                      <td className="px-8 py-5 text-center"><div className="flex justify-center gap-2"><button onClick={() => { setEditItem(k); setModalType('EDIT') }} className="p-2.5 bg-slate-50 text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-xl transition-all shadow-sm"><Edit3 size={16} /></button><button onClick={() => { setEditItem(k); setModalType('DELETE') }} className="p-2.5 bg-slate-50 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all shadow-sm"><Trash2 size={16} /></button></div></td>
                    </tr>
                  ))}
                </TacticalTable>
              )
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.98, opacity: 0 }} className="bg-white w-full max-w-xl rounded-[40px] shadow-2xl relative z-10 p-10">
               ...
               <div className="space-y-8">
                  {activeTab === 'WORDS' ? (
                    <>
                      <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-3 ml-1 tracking-widest">敏感词内容</label><input value={editItem?.word} onChange={(e)=>setEditItem({...editItem, word: e.target.value})} className="w-full px-6 py-4 bg-slate-50 rounded-2xl text-sm font-bold text-slate-900 shadow-inner border-none" /></div>
                      <div className="grid grid-cols-2 gap-6">
                        <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-3 ml-1 tracking-widest">风险权重</label><input type="number" value={editItem?.risk_level} onChange={(e)=>setEditItem({...editItem, risk_level: parseInt(e.target.value)})} className="w-full px-6 py-4 bg-slate-50 rounded-2xl text-sm font-black text-slate-900 shadow-inner border-none" /></div>
        ...
        {modalType === 'DELETE' && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.98, opacity: 0 }} className="bg-white w-full max-w-md rounded-[40px] shadow-2xl relative z-10 p-10 text-center">
               ...
               <h3 className="text-xl font-black text-slate-900 mb-2 italic">移除 AI 规则节点？</h3>
               <p className="text-xs text-slate-400 font-medium mb-8 leading-relaxed px-4">注销策略 <span className="text-red-600 font-black">[{editItem?.word || editItem?.keyword}]</span> 将导致 AI 引擎失去对应的识别能力。</p>
               <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => setModalType('NONE')} className="py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-xs uppercase hover:bg-slate-200 transition-all">放弃</button>
