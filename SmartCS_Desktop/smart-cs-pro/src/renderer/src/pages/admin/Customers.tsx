    ...
    <div className="space-y-6 h-full flex flex-col font-sans bg-slate-50/50 p-4 lg:p-6 text-slate-900">
      <header className="flex justify-between items-end bg-white p-8 rounded-xl border border-slate-200 shadow-sm shrink-0">
        ...
        {hasPermission('admin:customer:export') && (
          <button className="flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-xl text-xs font-black shadow-xl hover:bg-emerald-700 transition-all"><Download size={16} /> 导出全量战术报告</button>
        )}
      </header>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex gap-4 shrink-0">
        ...
        <button onClick={() => { setPage(1); fetchCustomers(false); }} className="p-3 bg-slate-50 text-slate-600 rounded-xl shadow-sm border border-slate-200 hover:bg-slate-100 active:scale-95 transition-all group">
              ...
              {customers.map((c, i) => (
                <motion.div key={c.id || i} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm hover:shadow-xl hover:border-cyan-200 transition-all group relative overflow-hidden bg-gradient-to-br from-white to-slate-50/50">
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div className={cn("w-14 h-14 rounded-xl flex items-center justify-center border", c.level === 'VIP' ? "bg-amber-50 text-amber-600 border-amber-100 shadow-inner" : "bg-cyan-50 text-cyan-600 border-cyan-100")}><UserCircle2 size={32} /></div>
                  ...
                  <div className="mt-6 space-y-4">
                    <div className="flex flex-wrap gap-1">{c.tags.split(',').map(t => (<span key={t} className="px-2 py-0.5 bg-white text-slate-600 text-[10px] font-black rounded-xl border border-slate-200 shadow-sm">{t}</span>))}</div>
