...
export default function NotificationsPage() {
  ...
  const markRead = async (id: string) => {
    const res = await window.api.callApi({ url: `${CONFIG.API_BASE}/admin/notifications/read`, method: 'POST', data: { id } })
  ...
  return (
    <div className="space-y-6 h-full flex flex-col font-sans bg-slate-50/50 p-4 lg:p-6">
      <div className="flex justify-between items-end bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm shrink-0">
        <div><h2 className="text-3xl font-black tracking-tight text-slate-900 uppercase italic">消息通知中心</h2><p className="text-slate-500 text-sm mt-1 font-medium">全域战术指令存档，确保每一条风险预警均已送达并阅知</p></div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
          {loading ? <div className="h-full flex items-center justify-center text-slate-400 gap-3"><Loader2 className="animate-spin" /><span>调取中枢存档...</span></div> : (
            <div className="grid grid-cols-1 gap-4">
              {notifs.map(n => (
                <div key={n.id} onClick={() => setSelectedMsg(n)} className={cn("p-6 rounded-[28px] border transition-all flex items-center gap-6 cursor-pointer", n.is_read ? "bg-slate-50/50 opacity-60 grayscale-[0.5]" : "bg-white border-slate-200 shadow-md hover:border-cyan-300 hover:shadow-2xl hover:scale-[1.01]")}>
                  <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border", n.is_read ? "bg-slate-100 text-slate-400 border-slate-200" : "bg-cyan-50 text-cyan-600 border-cyan-100 shadow-inner")}>
                  ...
                  {!n.is_read && <button onClick={(e) => {e.stopPropagation(); markRead(n.id)}} className="p-3 bg-cyan-500 text-slate-900 rounded-xl hover:bg-cyan-400 shadow-lg shadow-cyan-500/20"><CheckCircle2 size={18} /></button>}
                </div>
              ))}
            </div>
          )}
      ...
      <AnimatePresence>
        {selectedMsg && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedMsg(null)} className="absolute inset-0 bg-slate-950/60 backdrop-blur-md" />
            <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="relative w-full max-w-xl rounded-[40px] shadow-2xl overflow-hidden bg-white z-10 p-10">
               <div className="flex justify-between items-start mb-10"><div className="flex items-center gap-4"><div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg", selectedMsg.is_read ? "bg-slate-100 text-slate-400" : "bg-slate-900 text-cyan-400")}>{selectedMsg.is_read ? <MailOpen size={28} /> : <MessageSquare size={28} />}</div><div className="flex flex-col"><h3 className="text-2xl font-black text-slate-900 italic leading-none">{selectedMsg.title}</h3><span className="text-[10px] text-slate-400 font-black mt-2 uppercase tracking-widest">下发时间: {new Date(selectedMsg.created_at).toLocaleString()}</span></div></div><button onClick={() => setSelectedMsg(null)} className="p-2 hover:bg-slate-100 rounded-full transition-all text-slate-400"><X size={24}/></button></div>
               <div className="p-10 bg-slate-50 rounded-[32px] border border-slate-100 mb-10 shadow-inner"><p className="text-slate-700 text-lg font-medium leading-relaxed italic">"{selectedMsg.content}"</p></div>
               <div className="flex gap-4">
                  {selectedMsg.is_read ? (
                    <div className="flex-1 py-5 bg-emerald-50 text-emerald-600 rounded-[24px] font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 border border-emerald-100"><CheckCircle2 size={20} /> 指令已阅存档</div>
                  ) : (
                    <button onClick={() => markRead(selectedMsg.id)} className="flex-1 py-5 bg-slate-900 text-white rounded-[24px] font-black text-xs uppercase tracking-widest shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3"><ShieldAlert size={20} className="text-cyan-400" /> 确认指令并标记已阅</button>
                  )}
                  <button onClick={() => setSelectedMsg(null)} className="px-10 py-5 bg-slate-100 text-slate-500 rounded-[24px] font-black text-xs uppercase hover:bg-slate-200 transition-all">收起</button>
               </div>
            </motion.div>
          </div>
        )}
