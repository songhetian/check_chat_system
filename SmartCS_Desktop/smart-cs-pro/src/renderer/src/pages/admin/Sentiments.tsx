    ...
    <div className="flex flex-col gap-6 h-full font-sans bg-slate-50/50 p-4 lg:p-6 text-slate-900">
      <header className="flex justify-between items-end bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm shrink-0">
        ...
        <div className="flex gap-3">
           <button onClick={fetchData} className="p-3 bg-slate-50 text-slate-600 rounded-xl shadow-sm border border-slate-200 hover:bg-slate-100 transition-all"><RefreshCw size={18} className={cn(loading && "animate-spin")} /></button>
      ...
      <div className="flex-1 bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden flex flex-col">
         <div className="flex-1 overflow-y-auto custom-scrollbar">
            {loading ? <div className="h-64 flex flex-col items-center justify-center opacity-30"><Loader2 className="animate-spin mb-4" size={40} /><span className="text-xs font-black">调取中...</span></div> : (
                ...
                {data.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group text-sm font-bold text-slate-600 text-center">
                    <td className="px-8 py-5 font-black text-slate-900">
                       <span className={cn("px-3 py-1 rounded-lg border", `bg-${item.color}-50 text-${item.color}-600 border-${item.color}-100`)}>{item.name}</span>
                    </td>
                    ...
                    <td className="px-6 py-5">{item.is_active ? <span className="text-emerald-600">激活</span> : <span className="text-slate-300">禁用</span>}</td>
            ...
            <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.98, opacity: 0 }} className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl relative z-10 p-12">
               ...
               <div className="space-y-6">
                  ...
                  <button disabled={processing} onClick={handleSave} className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3">
                    {processing ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />} 固化维度逻辑
                  </button>
               </div>
            </motion.div>
          </div>
