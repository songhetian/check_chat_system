...
export default function CategoriesPage() {
  ...
  const handleSave = async () => {
    ...
    if (res.data.status === 'ok') {
      ...
      window.dispatchEvent(new CustomEvent('trigger-toast', { detail: { title: '指令已固化', message: '分类架构已实时更新', type: 'success' } }))
    }
  }
  ...
  const executeDelete = async () => {
    ...
    const res = await window.api.callApi({
      url: `${CONFIG.API_BASE}/ai/categories/delete`, // 需后端补全
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}` },
      data: { id: editItem.id }
    })
    setModalType('NONE'); fetchCats()
    window.dispatchEvent(new CustomEvent('trigger-toast', { detail: { title: '节点移除成功', message: '物理规则已重载', type: 'success' } }))
  }
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden z-10 bg-white p-10">
               ...
               <div className="space-y-8">
                  <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-3 ml-1">名称</label><input value={editItem?.name} onChange={(e)=>setEditItem({...editItem, name: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-black text-slate-900 shadow-inner" /></div>
                  <div><label className="text-[10px] font-black text-slate-400 uppercase block mb-3 ml-1">属性</label><TacticalSelect options={typeOptions} value={editItem?.type} onChange={(val) => setEditItem({...editItem, type: val})} /></div>
                  <button onClick={handleSave} className="w-full py-5 bg-slate-900 text-white rounded-[24px] font-black text-xs uppercase shadow-2xl active:scale-95 transition-all"><Save size={18} /> 确认并固化</button>
               </div>
            </motion.div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.98, opacity: 0 }} className="bg-white w-full max-w-md rounded-[40px] shadow-2xl relative z-10 p-10 text-center text-slate-900">
               ...
               <div className="grid grid-cols-2 gap-4">
                  ...
                  <button onClick={executeDelete} className="py-4 bg-red-500 text-white rounded-2xl font-black text-xs uppercase shadow-xl hover:bg-red-600 active:scale-95 transition-all">确认物理移除</button>
               </div>
            </motion.div>
          </div>
        )}
