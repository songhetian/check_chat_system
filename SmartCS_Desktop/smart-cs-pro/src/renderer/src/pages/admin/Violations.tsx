    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 text-black">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="relative w-full max-w-4xl bg-white rounded-xl shadow-xl overflow-hidden flex flex-col max-h-[90vh] z-10">
...
export default function ViolationsPage() {
  ...
  return (
    <div className="flex flex-col gap-6 h-full font-sans bg-slate-50/50 p-4 lg:p-6 text-black">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end bg-white p-6 rounded-xl border border-slate-200 shadow-sm shrink-0 gap-6">
        <div>
            ...
            <TacticalTable headers={['捕获时间', '成员姓名', '所属部门', '关键词', '风险等级', '操作']}>
              {data.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group cursor-pointer text-center" onClick={() => openEvidence(item)}>
                  ...
                  <td className="px-6 py-3 text-center"><div className="flex items-center justify-center gap-3"><div className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center font-black text-xs text-slate-600">{item.user__real_name?.[0] || '?'}</div><span className="text-xs font-black text-black">{item.user__real_name}</span></div></td>
                  ...
                  <td className="px-8 py-3 text-center"><button onClick={(e) => { e.stopPropagation(); openEvidence(item); }} className="p-2 bg-slate-100 text-black hover:bg-black hover:text-white rounded-xl transition-all"><ArrowUpRight size={14} /></button></td>
                </tr>
              ))}
            </TacticalTable>
          )}
