  ...
  return (
    <div className="flex flex-col gap-6 h-full font-sans bg-slate-50/50 p-4 lg:p-6 text-slate-900">
      <header className="flex justify-between items-end bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm shrink-0">
        <div>
          <h2 className="text-3xl font-black text-slate-900 uppercase italic text-tactical-glow">组织架构中枢</h2>
          <p className="text-slate-500 text-sm mt-1 font-medium flex items-center gap-2">
            精确定义业务战术单元，实现全域数据隔离与动作权限管控
      ...
      <div className="bg-white p-4 rounded-[24px] border border-slate-200 shadow-sm flex flex-wrap items-center gap-4 shrink-0">
        ...
        <button onClick={() => deptsQuery.refetch()} className="p-3 bg-slate-50 text-slate-600 rounded-2xl shadow-sm border border-slate-200 hover:bg-slate-100 active:scale-95 transition-all group">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {deptsQuery.isLoading ? (
            <div className="h-64 flex items-center justify-center text-slate-400 gap-3 font-bold uppercase italic tracking-widest">
              <Loader2 className="animate-spin" /> 同步架构中...
            </div>
          ) : (
            <TacticalTable headers={['部门名称', '部门人数', '部门主管', '运行状态', '战术动作']}>
              {depts.map((d: any) => (
                <tr key={d.id} className="hover:bg-slate-50/50 transition-colors group text-center">
                  <td className="px-8 py-5 text-left"><div className="flex items-center gap-3"><div className={cn("w-10 h-10 rounded-xl flex items-center justify-center border shadow-inner", d.manager__real_name ? "bg-cyan-50 text-cyan-600 border-cyan-100" : "bg-slate-50 text-slate-400 border-slate-100")}><Building2 size={18} /></div><span className="text-sm font-black text-slate-900">{d.name}</span></div></td>
                  <td className="px-6 py-5 font-bold text-slate-600"><div className="flex items-center justify-center gap-2"><Users2 size={14} className="text-slate-300" /><span className="text-sm">{d.member_count} <span className="text-[10px] opacity-40 italic">UNIT</span></span></div></td>
                  <td className="px-6 py-5">{d.manager__real_name ? <div className="flex items-center justify-center gap-2"><div className="w-6 h-6 rounded-full bg-slate-900 text-white flex items-center justify-center text-[9px] font-black shadow-sm">{d.manager__real_name[0]}</div><span className="text-xs font-black text-slate-700">{d.manager__real_name}</span></div> : <span className="text-[10px] text-slate-300 font-bold italic">未指派</span>}</td>
                  <td className="px-6 py-5 font-black italic text-[10px] text-emerald-600 uppercase tracking-widest">Active</td>
      ...
      <PerformanceModal isOpen={modalType === 'ADD' || modalType === 'EDIT'} onClose={() => setModalType('NONE')} title={modalType === 'ADD' ? '录入新战术单元' : '调整组织架构'} isPending={saveMutation.isPending}>
        <div className="space-y-6">
          <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2 ml-1">部门标识名称</label><input disabled={saveMutation.isPending} autoFocus value={inputName} onChange={(e) => setInputName(e.target.value)} placeholder="输入名称..." className="w-full px-5 py-3.5 bg-slate-50 border-none rounded-xl text-sm font-black text-slate-900 shadow-inner disabled:opacity-50" /></div>
          {modalType === 'EDIT' && (
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block pl-1">指派实战主管</label>
              <div className="relative group"><div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-cyan-500 transition-colors"><Search size={14} /></div><input disabled={saveMutation.isPending} value={userSearch} onChange={(e) => setUserSearch(e.target.value)} placeholder="检索姓名..." className="w-full pl-10 pr-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-bold text-slate-900 shadow-inner disabled:opacity-50" /></div>
              <div className="max-h-36 overflow-y-auto custom-scrollbar border border-slate-100 rounded-xl p-1.5 bg-slate-50/30">
                 {deptUsers.map((u: any) => (
                   <div key={u.id} onClick={() => !saveMutation.isPending && setManagerId(u.id)} className={cn("p-2.5 rounded-lg cursor-pointer flex justify-between items-center transition-all mb-1 last:mb-0", managerId == u.id ? "bg-cyan-500 text-white shadow-md" : "hover:bg-white text-slate-600", saveMutation.isPending && "opacity-50 cursor-not-allowed")}>
      ...
      <PerformanceModal isOpen={modalType === 'DELETE'} onClose={() => setModalType('NONE')} title="物理注销确认" isPending={deleteMutation.isPending}>
        <div className="space-y-6 flex flex-col items-center text-center">
           ...
           <div><h4 className="text-lg font-black text-slate-900 mb-1 italic text-center">确认物理注销该部门？</h4><p className="text-[11px] text-slate-500 font-medium leading-relaxed px-4 text-center">注销 <span className="text-red-600 font-black">[{targetItem?.name}]</span> 将导致该单元关联的所有业务链条断裂。</p></div>
           <div className="grid grid-cols-2 gap-3 w-full">
              <button disabled={deleteMutation.isPending} onClick={() => setModalType('NONE')} className="py-3.5 bg-slate-100 text-slate-600 rounded-xl font-black text-[10px] uppercase hover:bg-slate-200 transition-all disabled:opacity-50">放弃动作</button>
              <button disabled={deleteMutation.isPending} onClick={() => deleteMutation.mutate(targetItem.id)} className="py-3.5 bg-red-500 text-white rounded-xl font-black text-[10px] uppercase shadow-lg hover:bg-red-600 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
