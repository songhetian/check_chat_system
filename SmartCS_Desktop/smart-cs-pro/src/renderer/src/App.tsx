            ...
            {activeCommand && (
              <motion.div initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} className="fixed inset-0 z-[200] flex items-center justify-center p-12">
                <div className="w-full max-w-2xl bg-slate-900 border-4 border-cyan-500 rounded-[40px] p-10 shadow-[0_0_100px_rgba(6,182,212,0.5)] flex flex-col items-center text-center space-y-6">
                   <div className="w-24 h-24 bg-cyan-500/10 rounded-full flex items-center justify-center text-cyan-400 border-2 border-cyan-500/20 animate-pulse"><ShieldAlert size={48} /></div>
                   <div className="space-y-2"><h2 className="text-[12px] font-black text-cyan-500 uppercase tracking-[0.3em]">收到指挥官远程指令</h2><h1 className="text-4xl font-black text-white tracking-tighter">{activeCommand.title}</h1></div>
                  ...
                  <div className="space-y-6">
                    <div className="flex justify-between items-end">
                      ...
                      <div className="flex gap-3">
                         ...
                         <div className="bg-green-500/10 text-green-600 px-4 py-2 rounded-xl border border-green-500/20 text-xs font-bold flex items-center gap-2"><div className="w-2 h-2 bg-green-500 rounded-full animate-ping" /> 系统链路极度稳定</div>
                      </div>
                    </div>
                      ...
                      {['张三', '李四', '王五', '赵六', '陈七'].map((name, i) => (
                        <motion.div key={name} whileHover={{ y: -5 }} className="bg-white p-5 rounded-[32px] border border-slate-200 shadow-sm relative overflow-hidden group">
                          ...
                          <div className="pt-4 border-t border-slate-50 flex items-center justify-between">
                             <div className="flex flex-col"><span className="text-[8px] font-bold text-slate-400 uppercase">当前窗口</span><span className="text-[10px] font-black text-slate-700 truncate w-24">{i === 1 ? '微信 - 争议处理' : '钉钉 - 客户接待'}</span></div>
