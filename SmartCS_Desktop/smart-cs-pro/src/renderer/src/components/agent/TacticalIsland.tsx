...
export const TacticalIsland = () => {
  ...
  const mockCustomers = useMemo(() => [
    {
      ...
      attitude: "A+ 极度友好",
      insights: [
        { title: "极强购买力", desc: "对旗舰系列极度敏感，建议优先推介。", color: "emerald", icon: <Wallet size={16}/> },
  ...
  return (
    <div className="h-screen w-screen flex flex-col items-center justify-center overflow-hidden pointer-events-none select-none bg-transparent">
      ...
      <AnimatePresence>
        {showCalibration && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] bg-black/60 backdrop-blur-md flex flex-col items-center justify-center pointer-events-auto cursor-crosshair">
             <div className="absolute top-10 flex flex-col items-center gap-4">
                <div className="px-8 py-3 bg-cyan-500 text-slate-950 rounded-2xl font-black text-sm uppercase shadow-2xl">战术物理校准模式 : 已激活</div>
                <p className="text-white/60 text-xs font-black uppercase tracking-[0.3em]">请点击并拖选聊天工具的对话文本区域</p>
             </div>
             <div className="w-[400px] h-[300px] border-2 border-cyan-500 border-dashed rounded-3xl flex items-center justify-center bg-cyan-500/5 relative">
                ...
                <Crosshair size={48} className="text-cyan-500/40 animate-pulse" />
             </div>
             <button onClick={() => setShowCalibration(false)} className="mt-10 px-10 py-4 bg-white text-slate-950 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl active:scale-95 transition-all">保存校准数据并回归</button>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <div className="flex items-center px-4 h-[64px] shrink-0 cursor-move relative" style={{ WebkitAppRegion: 'drag' } as any}>
            <div className="flex items-center gap-2 w-[110px] shrink-0">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-[14px] font-black text-white truncate leading-none mb-0.5">{user?.real_name || '实战节点'}</span>
            ...
            <div className="flex-1 flex items-center justify-center gap-1.5" style={{ WebkitAppRegion: 'no-drag' } as any}>
              ...
              <HubBtn icon={<Package size={16} />} active={activeSideTool === 'PRODUCTS'} onClick={() => { setLayoutMode('SIDE'); setActiveSideTool('PRODUCTS' as any); }} title="商品" color="white" />
              <HubBtn icon={<BookOpen size={16} />} active={activeSideTool === 'KNOWLEDGE'} onClick={() => { setLayoutMode('SIDE'); setActiveSideTool('KNOWLEDGE' as any); }} title="知识" color="white" />
              ...
              <div className="w-px h-4 bg-white/10 mx-0.5" />
              <HubBtn icon={<Globe size={16} />} active={showBigScreenModal} onClick={() => setShowBigScreenModal(!showBigScreenModal)} title="全景" color="emerald" />
              <HubBtn icon={<Hand size={16} />} active={showHelpModal} onClick={() => setShowHelpModal(!showHelpModal)} title="求助" color="red" />
              <HubBtn icon={<LayoutGrid size={16} />} active={isExpanded} onClick={() => setIsExpanded(!isExpanded)} title="功能" color="muted" />
          ...
          <div className="flex-1 flex flex-col bg-slate-950 overflow-hidden" style={{ WebkitAppRegion: 'no-drag' } as any}>
             <div className="p-8 bg-cyan-600 text-white flex justify-between items-center shrink-0">
                ...
                <div className="flex flex-col items-end">
                   <h4 className="text-xl font-black italic tracking-tighter uppercase">{activeSideTool === 'PRODUCTS' ? '实战资产库' : (activeSideTool === 'KNOWLEDGE' ? '战术知识库' : '实时客户洞察')}</h4>
                   <div className="flex items-center gap-1.5 opacity-60"><ScanEye size={10}/><span className="text-[8px] font-black uppercase tracking-widest">神经链路同步中</span></div>
                </div>
             </div>
             ...
             {activeSideTool !== 'CUSTOMERS' ? (
               <div className="flex-1 flex flex-col">
                  <div className="p-8 bg-black/40 space-y-6 border-b border-white/10">
                     <div className="relative">
                        ...
                        <input value={searchQuery} onChange={(e) => setSearchText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && executeSearch()} placeholder={`输入关键字搜索并按回车...`} className="w-full bg-white/5 border-2 border-white/10 rounded-[24px] py-5 pl-14 pr-8 text-base text-white focus:border-cyan-500 outline-none transition-all font-black" />
                     </div>
                     {activeSideTool === 'PRODUCTS' && (
                       <div className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl border border-white/5"><Wallet size={16} className="text-cyan-500" /><input placeholder="最高单价筛选" value={filterPrice} onChange={e => setFilterPrice(e.target.value)} className="bg-transparent w-full text-sm outline-none text-white font-black" /></div>
                     )}
                  </div>
                     ...
                     {searchResults.map((item, i) => (
                       <div key={i} className={cn("p-6 rounded-[32px] bg-white/[0.03] border transition-all relative overflow-hidden", activeSideTool === 'PRODUCTS' ? "border-emerald-500/20 hover:border-emerald-500/50" : "border-cyan-500/20 hover:border-cyan-500/50")}>
                          <div className="text-[10px] font-black uppercase mb-3 tracking-[0.2em] flex justify-between items-center">
                             <span className={activeSideTool === 'PRODUCTS' ? "text-emerald-500" : "text-cyan-500"}>{activeSideTool === 'PRODUCTS' ? 'EQUIPMENT ASSET' : 'TACTICAL MANUAL'}</span>
               ...
               <div className="flex-1 overflow-y-auto p-8 space-y-8 animate-in fade-in slide-in-from-right-4">
                  <div className="p-8 rounded-[40px] bg-emerald-500 text-slate-950 shadow-2xl relative overflow-hidden">
                     ...
                     <div className="flex items-center gap-4 mb-6"><div className="w-14 h-14 rounded-2xl bg-black/20 flex items-center justify-center border border-black/10"><UserIcon size={32}/></div><div><h2 className="text-3xl font-black italic tracking-tighter mb-1">{currentCustomer?.name}</h2><span className="text-[10px] font-black uppercase bg-black/10 px-2 py-0.5 rounded-md">{currentCustomer?.level}</span></div></div>
                     <div className="grid grid-cols-2 gap-4"><div className="bg-black/10 p-4 rounded-3xl border border-black/5"><p className="text-[10px] font-black opacity-60 uppercase mb-1">历史贡献值</p><p className="text-2xl font-black italic">¥{currentCustomer?.total_value}</p></div><div className="bg-black/10 p-4 rounded-3xl border border-black/5"><p className="text-[10px] font-black opacity-60 uppercase mb-1">态度指数</p><p className="text-2xl font-black italic">{currentCustomer?.attitude}</p></div></div>
                  </div>
                  <div className="flex gap-2 p-2 bg-white/5 rounded-2xl border border-white/5"><button onClick={() => { setCurrentCustomer(mockCustomers[0]); setShowCriticalAlert(false); }} className={cn("flex-1 py-2 rounded-xl text-[10px] font-black uppercase transition-all", currentCustomer?.name === '李先生' ? "bg-emerald-500 text-black" : "bg-white/5 text-slate-500")}>模拟 VIP</button><button onClick={() => { setCurrentCustomer(mockCustomers[1]); setShowCriticalAlert(true); }} className={cn("flex-1 py-2 rounded-xl text-[10px] font-black uppercase transition-all", currentCustomer?.name === '未知用户' ? "bg-red-500 text-white" : "bg-white/5 text-slate-500")}>模拟 恶劣客户</button></div>
                  <div className="space-y-4">
                     <h5 className="text-[11px] font-black text-emerald-500 uppercase tracking-[0.4em] flex items-center gap-2 px-2"><ScanEye size={16}/> 实战风险洞察</h5>
                     {currentCustomer?.insights.map((ins: any, idx: number) => (<InsightCard key={idx} title={ins.title} desc={ins.desc} color={ins.color} icon={ins.icon} />))}
                  </div>
               </div>
             )}
          </div>
              ...
              <div className="flex flex-col gap-4 flex-1 min-h-0">
                 ...
                 <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3">
                    {helpSuggestions.length > 0 && (
                       <div className="p-4 rounded-3xl bg-emerald-500/5 border border-emerald-500/20"><h5 className="text-[10px] font-black text-emerald-500 uppercase flex items-center gap-2 mb-3 tracking-widest"><SearchCheck size={14}/> 匹配方案推荐 · 优先尝试自救</h5>
                          {helpSuggestions.map((s, i) => (<div key={i} className="mb-2 last:mb-0 p-4 bg-black/40 rounded-2xl border border-white/5"><div className="text-[12px] font-black text-white mb-1">对策: {s.keyword}</div><div className="text-[11px] text-emerald-400 font-medium italic">方案: {s.solution}</div></div>))}
                       </div>
                    )}
                 </div>
              </div>
...
function InsightCard({ title, desc, color, icon }: any) {
  ...
  return (<div className={cn("p-5 rounded-3xl border flex gap-4 transition-all hover:bg-white/5", colors[color])}><div className="shrink-0 mt-1">{icon}</div><div><p className="text-sm font-black mb-1">{title}</p><p className="text-[11px] font-medium opacity-80">{desc}</p></div></div>)
}